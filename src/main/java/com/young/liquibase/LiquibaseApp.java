...
public class LiquibaseApp extends Main{
...
//            }
            ...
            LiquibaseApp main = new LiquibaseApp();
            ...
            if ("update".equals(main.command)) {
                System.err.println("Liquibase 更新成功");
            } else if (main.command.startsWith("rollback") && !main.command.endsWith("SQL")) {
                System.err.println("Liquibase 回滚成功");
            } else if (!main.command.endsWith("SQL")) {
                System.err.println("Liquibase '" + main.command + "' 成功");
            }
        } catch (Throwable e) {
            ...
            try {
                if (e.getCause() instanceof ValidationFailedException) {
                    ...
                    System.err.println("Liquibase运行时发生错误: " + message + "\n");
            ...
            throw new LiquibaseException("Liquibase运行时发生错误: " + message, e);
        }
    }
    ...
    protected List<String> checkSetup() {
        ...
        if (command == null) {
            ...
            if (isChangeLogRequired(command) && changeLogFile == null) {
                messages.add("--changeLogFile 必填");
            }

            if (isNoArgCommand(command) && !commandParams.isEmpty()) {
                messages.add("未知的命令参数: " + commandParams);
            } else {
    ...
    private void checkForMissingCommandParameters(final List<String> messages) {
        if (commandParams.isEmpty() || commandParams.iterator().next().startsWith("-")) {
            if ("calculateCheckSum".equalsIgnoreCase(command)) {
                messages.add("丢失 changeSet identifier");
            }
        }
    }
...
//            	referencePassword = new String(System.console().readPassword("Reference DB Password:"));
    ...
    protected ResourceAccessor getFileOpen(){
        FileSystemResourceAccessor fsOpener = null;
        if(this.baseDir==null){
            fsOpener = new FileSystemResourceAccessor();
        }else{
            fsOpener = new FileSystemResourceAccessor(baseDir);
        }
    ...
    protected void doMigration() throws Exception {
        ...
        ResourceAccessor fileOpener = getFileOpen();
